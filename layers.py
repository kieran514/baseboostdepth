...
class BackprojectDepth(nn.Module):
    ...
    def forward(self, depth, inv_K):
        size = len(inv_K)
        # size = inv_K.shape[0]
        cam_points = torch.matmul(inv_K[:, :3, :3], self.pix_coords[:size])
        cam_points = depth.view(size, 1, -1) * cam_points
        cam_points = torch.cat([cam_points, self.ones[:size]], 1)
...
def compute_depth_errors(gt, pred, mask=None, SYNS=False):
    ...
    if SYNS:
        ...
        D_pred = ndimage.distance_transform_edt(1 - pred[:,:,0]) 

        pred_edges = pred[:,:,0] & (D_target < th_edges)  
